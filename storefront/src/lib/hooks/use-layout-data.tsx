import { getProductsList, getCollectionsList, getLatestPost } from "@lib/data"
...
const fetchLatestPost = async (): Promise<PostType[]> => {
  const posts: PostType[] = await getLatestPost();
  console.log('fetchLatestPost returns: ', posts);
  return posts;
}

export const useLatestPost = async () => {
  const queryResults = await fetchLatestPost();
  console.log('useLatestPost returns: ', queryResults);
  return queryResults;
}
